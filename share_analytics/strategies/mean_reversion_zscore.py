...
class MeanReversionZScoreStrategy(Strategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        buy_signal = (signals["zscore"] < self.entry_z) & (previous_zscore.fillna(self.entry_z) >= self.entry_z)
        sell_signal = (signals["zscore"] > self.exit_z) & (previous_zscore.fillna(self.exit_z) <= self.exit_z)
